...
def kmeans_segmentation_gray_scale(image_path, num_clusters, output_path):
    # Abre a imagem
    image = Image.open(image_path)
    ...
    # Redimensiona o array para que cada pixel seja uma amostra
    image_array_flat = image_array.reshape((-1, 1))  # Apenas uma dimensão para tons de cinza
...
for image_path in images_path:
    ...
    output_path_gray = f"{masks_dir}/{image_path.split('.')[0]}_mask_gray.{image_path.split('.')[1]}"
    ...
    kmeans_segmentation_gray_scale(f'{images_dir}/{image_path}', num_clusters, output_path_gray)
